...
const discord_token = "NTQ2MzQ3OTg1MTQ4MTgyNTU5.D2lCQg.1rGRW7oaIPjhu46j9vd1CGWI_Ug";
...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = [`${client.guilds.size} Server`,' M!Bot Credit System','Type -help |-support |-invite ',`${client.users.size} Members`,'-invite','By: FBIGamer & Moscow | FBIGamer '];
    var i = -1;
    var j = 0;
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[i],`http://www.twitch.tv/FBIGamer`);
    }, ms);
...
client.on("message", message => {
 if (message.content === "-invite") {
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setFooter('© M!Bot :heart: جميع الحقوق محفوظة 2018 لــبوت')
      .addField('شكرا لك لاستخدامك ل بروميوم بوت', `Soon`)
...
console.log(` M!Bot  Added To Server ${guild.name} , The Owner Is ${guild.owner.user.username} , Members In Server : **[${guild.memberCount}]**            `)
client.channels.get("489944931868082177").send('** :purple_heart: M!Bot ** ``Added``:white_check_mark:  To Server '+`** [ ${guild.name} ] **`+''+'  The Owner Is  ' +`**[ ${guild.owner.user.username} ]** , Members In Server : **[${guild.memberCount}]**` +'')
});

client.on("guildDelete", guild => {
  console.log(`  M!Bot  Leave From Server ${guild.name}, The Server Owner Is ${guild.owner.user.username}`)
  client.channels.get("489944931868082177").send('** :purple_heart: M!Bot  **``Kicked``:x:  From Server '+`** [ ${guild.name} ] **`+''+' The Owner Is ' +`**[ ${guild.owner.user.username} ]**` +'')
  });
